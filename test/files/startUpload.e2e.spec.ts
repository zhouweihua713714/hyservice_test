...
import { StartUploadDto } from '@/modules/files/files.dto';
...
describe('test/files/startUpload.e2e.spec.ts', () => {
  // 引入全局变量
  ...
  test('should Post /file/startUpload without exercise detail id ', async () => {
    // make request
    const result = await request(app.getHttpServer()).post('/file/startUpload').send(body);
    // use expect by jest
    expect(result.status).toBe(HttpStatus.OK);
    expect(result.body.code).toBe(200);
    expect(result.body.message).toBe('ok');
    expect(result.body.data.answerImages).toEqual([]);
    // 状态键
    ...
    expect(statusInfo.status).toBe(Upload_Status_Enum.Uploading);
  });

  test('should Post /file/startUpload with exercise detail id ', async () => {
    // make request
    const result = await request(app.getHttpServer()).post('/file/startUpload').send({
      uploadToken: body.uploadToken,
      id: userExerciseDetailsRet.user_exercise_details[0].id,
    });
    // use expect by jest
    expect(result.status).toBe(HttpStatus.OK);
    expect(result.body.code).toBe(200);
    expect(result.body.message).toBe('ok');
    expect(result.body.data.answerImages).toEqual(['123.jpg']);
    // 状态键
    const statusKey = getRedisKey(RedisKeyPrefix.UPLOAD_TOKEN, body.uploadToken || '');
    // 查询状态
    const statusJsonInfo = await redisService.get(statusKey);
