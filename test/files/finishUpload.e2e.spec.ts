...
import { FinishUploadDto } from '@/modules/files/files.dto';
...
describe('test/files/finishUpload.e2e.spec.ts', () => {
  // 引入全局变量
  ...
  beforeEach(async () => {
    ...
    uploadTokenRet = await createUploadTokenOfCache(
      ...
      config,
      redisService,
      Upload_Status_Enum.Uploading
    );
    body.uploadToken = uploadTokenRet.uploadToken;
  });

  test('should not Post /file/finishUpload when uploadToken expired', async () => {
    const statusKey = uploadTokenRet.statusKey;
    await redisService.del(statusKey);
    // make request
    const result = await request(app.getHttpServer()).post('/file/finishUpload').send(body);
    // use expect by jest
    expect(result.status).toBe(HttpStatus.UNAUTHORIZED);
  });
  test('should Post /file/finishUpload', async () => {
    // make request
    const result = await request(app.getHttpServer()).post('/file/finishUpload').send(body);
    // use expect by jest
    ...
    expect(statusInfo.answerImages).toBeTruthy();
    expect(statusInfo.status).toBe(Upload_Status_Enum.Finished);
  });
