...
import { DataType } from './operateAnalysisPolicies.seed';

const tester = new DBTester<DataType>().setup();

describe('/policies/operateAnalysisPolicies', () => {
  test('should not POST before login', async () => {
    const result = await request(tester.server)
      .post('/policies/operateAnalysisPolicies')
      .send({ ids: ['-1'], status: Content_Status_Enum.ACTIVE });
    expect(result.status).toBe(HttpStatus.FORBIDDEN);
  });
  test('should not POST /policies/operateAnalysisPolicies with user is not admin', async () => {
    const result = await request(tester.server)
      .post('/policies/operateAnalysisPolicies')
  ...
  test('should POST /policies/operateAnalysisPolicies with status is active', async () => {
    // save with
    const result = await request(tester.server)
      .post('/policies/operateAnalysisPolicies')
  ...
  test('should POST /policies/operateAnalysisPolicies with status is ready', async () => {
    // save with
    const result = await request(tester.server)
      .post('/policies/operateAnalysisPolicies')
      .set('Authorization', tester.data.user.headers.authorization)
