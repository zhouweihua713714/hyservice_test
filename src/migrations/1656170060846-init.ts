...
export class init1656170060846 implements MigrationInterface {
    name = 'init1656170060846'
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`CREATE TABLE "files" ("id" uuid NOT NULL DEFAULT uuid_generate_v4(), "filename" text NOT NULL, "status" smallint NOT NULL DEFAULT '0', "oss_info" jsonb, "created_at" TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now(), CONSTRAINT "PK_6c16b9093a142e0e7613b04a3d9" PRIMARY KEY ("id"))`);
        await queryRunner.query(`CREATE UNIQUE INDEX "files_pkey" ON "files" ("id") `);
        await queryRunner.query(`CREATE TABLE "users" ("id" uuid NOT NULL DEFAULT uuid_generate_v4(), "mobile" character varying(11) NOT NULL, "name" character varying(20), "info" json NOT NULL DEFAULT '{}', "created_at" date NOT NULL DEFAULT now(), "gender" character varying(10) NOT NULL DEFAULT 'unknown', "status" character varying(10) NOT NULL DEFAULT 'enabled', CONSTRAINT "UQ_d376a9f93bba651f32a2c03a7d3" UNIQUE ("mobile"), CONSTRAINT "PK_a3ffb1c0c8416b9fc6f907b7433" PRIMARY KEY ("id")); COMMENT ON COLUMN "users"."mobile" IS '手机号码'; COMMENT ON COLUMN "users"."name" IS '姓名'; COMMENT ON COLUMN "users"."info" IS '用户拓展信息'; COMMENT ON COLUMN "users"."created_at" IS '创建时间'; COMMENT ON COLUMN "users"."gender" IS '性别:男:male,女:female,未知:unknown'; COMMENT ON COLUMN "users"."status" IS '是否有效:enabled,disabled 无效'`);
